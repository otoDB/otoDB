...
model_classes_with_history = {
	'mediawork': [
		('mediawork', 'id'),
		('mediawork', 'moved_to__id'),
		('workrelation', 'A__id'),
		('workrelation', 'B__id'),
		('worksource', 'media__id'),
	],
	'mediasong': [
		('mediasong', 'id'),
		('songrelation', 'A__id'),
		('songrelation', 'B__id'),
		('mediasongconnection', 'song__id'),
	],
	'tagwork': [
		('tagwork', 'id'),
		('tagwork', 'aliased_to__id'),
		('wikipage', 'tag__id'),
		('tagworkconnection', 'tag__id'),
		('tagworkmediaconnection', 'tag__id'),
		('tagworkcreatorconnection', 'tag__id'),
		('tagworklangpreference', 'tag__id'),
		('tagworkparenthood', 'tag__id'),
	],
	'tagsong': [('tagsong', 'id')],
}
models_with_history = model_classes_with_history.keys()
model_classes_reverse = {
	cc[0]: k for k, c in model_classes_with_history.items() for cc in c
}
...
def get_history_querysets(**kwargs):
	return {
		model: union_qs(
			[
				ContentType.objects.get(model=c)
				.model_class()
				.history.order_by()
				.filter(history_user__isnull=False, **kwargs)
				.annotate(model=Value(c), instance=F(key))
				.values(
					'history_id',
					'history_date',
		...
		for model, cc in model_classes_with_history.items()
	}
...
def get_instance_history_querysets(pk):
	return {
		model: [
			...
			.model_class()
			.history.order_by()
			.filter(history_user__isnull=False, **{key: pk})
			.annotate(
				model=Value(c),
			)
			for c, key in cc
...
def resolve_history_instance_id(qs, n: int):
	...
	for q in qs:
		if i >= n:
			break
		instance_target_model = ContentType.objects.get(
			model=model_classes_reverse[q['model']]
		).model_class()
		try:
...
def history(
	request: HttpRequest, pk: int | str, model: Literal[*models_with_history], **kwargs
):
	if model == 'tagwork' or model == 'tagsong':
		pk = get_object_or_404(
			ContentType.objects.get(model=model).model_class().objects, slug=pk
		).pk
	historicals = [
		list(qs.order_by('-history_date')[: kwargs['pagination_info'].limit])
		for qs in get_instance_history_querysets(pk)[model]
	]
	assert historicals
...
def user(request: HttpRequest, username: str, **kwargs):
	return resolve_history_instance_id(
		get_combined_history_queryset(history_user__username=username).order_by(
			'-history_date'
		),
		kwargs['pagination_info'].limit,
	)
...
def rollback(
	request: HttpRequest, model: Literal[*model_classes_reverse.keys()], history_id: int
):
	ContentType.objects.get(model=model).model_class().history.get(
