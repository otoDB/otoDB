...
def set_tags(request: AuthedHttpRequest, work_id: int, payload: List[str]):
	work = get_object_or_404(MediaWork.active_objects, id=work_id)
...
def update_creator_roles(request: AuthedHttpRequest, payload: CreatorRolesUpdateSchema):
...
def toggle_sample(request: AuthedHttpRequest, work_id: int, tag_slug: str):
	instance = get_object_or_404(
...
def relation(request: AuthedHttpRequest, payload: RelationSchema):
...
def delete_relation(request: AuthedHttpRequest, A: int, B: int):
