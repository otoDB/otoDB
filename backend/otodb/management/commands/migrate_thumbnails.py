...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Starting thumbnail migration...')
        )
        ...
        for offset in range(0, total_works, batch_size):
            ...
            for work in batch:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Unexpected error processing Work {work.pk}: {e}")
                    )
                    error_count += 1
    ...
    def migrate(self, work: MediaWork, dry_run: bool = False) -> bool:
        ...
        for source in work_sources:
            if dry_run:
                self.stdout.write(f"  [DRY RUN] Would migrate to WorkSource {source.pk}")
                return True

            if not source.thumbnail_mime:
                try:
                    response = requests.get(work._thumbnail, allow_redirects=True, timeout=10)
                    content_type = response.headers.get('Content-Type')
                    source.thumbnail_mime = MimeType.from_str(content_type)

                    if not source.thumbnail_mime:
                        self.stdout.write(f"  Could not detect MIME type for {work._thumbnail}")
                        return False
                except Exception as e:
                ...
                if not success:
                    self.stdout.write(f"  Failed to upload thumbnail for WorkSource {source.pk}")
                    return False
        ...
        with transaction.atomic():
            ...
            self.stdout.write(f"  Set thumbnail_source to WorkSource {work.thumbnail_source.pk}")  # type: ignore
