...
{#snippet link(pathname: string, title: string)}
	<li>
		<a
			href={pathname}
			class="aria-[current=page]:text-otodb-content-fainter text-sm no-underline"
...
<nav class={[props.class]}>
	<div class="flex w-full flex-col gap-y-2">
				...
				{:else}
					...
					<li class="mt-4">
						<form method="POST" action="/logout">
							<button
								type="submit"
								class="w-full cursor-pointer border-none bg-transparent p-0 text-left text-sm text-[inherit] no-underline"
							>
		...
		{/if}
		<div class="border-otodb-content-faint bg-otodb-bg-faint/75 block border px-3 py-2">
			<div class="border-otodb-content-faint mb-2 border-b text-xs">
