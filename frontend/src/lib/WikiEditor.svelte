<script lang="ts">
	...
	let {
		...
		wiki_page: components['schemas']['WikiPageMDSchema'][];
		withTitle?: boolean;
	} = $props();

	let title = $state(wiki_page.find((p) => p.title)?.title ?? '');
	let wikiView = $state(getLocale());
	let mds = $state(
		Object.fromEntries(
			locales.map((lang) => [
				lang,
				wiki_page.find((p) => p.lang === languages[lang].id)?.page ?? ''
			])
		)
	);
...
{#if withTitle}
	<table class="mb-3">
		<tbody>
			<tr>
				<th><label for="wiki-title">Title</label></th>
				<td><input id="wiki-title" type="text" name="title" bind:value={title} required /></td>
			</tr>
		</tbody>
	</table>
{/if}
