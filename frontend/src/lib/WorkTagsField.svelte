...
<span role="none">
    <textarea onkeyup={() => { updateValue(); search(); }} onclick={search} bind:this={textarea} {...props}></textarea>
    <ul class="absolute" use:clickOutside onOutclick={() => { suggestions = []; }}>
        {#each suggestions as t}
            <li><a href={null} onclick={() => { textarea.value = replaceWordAtPos(textarea.value, textarea.selectionStart, t + ' '); suggestions = []; updateValue(); }}>{t}</a></li>
        {/each}
    </ul>
</span>
