<script lang="ts">
	...
	import DesktopGlobalNav from '$lib/GlobalNav/Desktop.svelte';
	import MobileGlobalNav from '$lib/GlobalNav/Mobile.svelte';
...
<div class="text-otodb-content-primary overflow-auto">
	...
	<div class="absolute z-[9999] md:hidden">
		<!-- Hamburger button -->
		<button
			class={[
				'bg-otodb-bg-primary/90 fixed bottom-[32px] left-[32px] z-1 h-12 w-12',
		...
		<button
			class={[
				'fixed inset-0 z-1 cursor-default cursor-pointer border-none bg-black/75 p-0',
		...
		<MobileGlobalNav
			...
			user={data.user}
			class={[
				'fixed top-0 left-0 z-2 z-50 h-full ',
				isMobileNavOpen ? 'visible' : 'invisible'
			]}
			close={() => {
				closeMobileNav();
	...
	<div class="relative mx-auto w-full gap-x-4 px-4 md:flex">
		<DesktopGlobalNav
			stats={data.stats}
			user={data.user}
			class="hidden max-w-[85vw] shrink-0 md:block"
		/>
		<div class="grow">
			<main id="content">
