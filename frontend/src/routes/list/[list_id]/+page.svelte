...
<Section title={m.mild_loud_shad_enchant({ type: m.stale_loose_squid_cut(), name: data.list.name})}>
    ...
    <a href="/list/{data.list.id}/delete" data-sveltekit-preload-data="tap">Delete this list...</a>
    {/if}
</Section>
<Section title="Entries">
    ...
    <li class="p-2 ml-5">
        <div style="display: flex; gap: 1rem;align-items:flex-start;">
            <a href="/work/{entry.work.id}"><img style="max-width:10rem" src={entry.work.thumbnail} alt={entry.work.title}></a>
            <div><a href="/work/{entry.work.id}">{entry.work.title}</a><p>{entry.description}</p></div>
        </div>
    </li>
    {:else}
