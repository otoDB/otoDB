...
export const load: LayoutServerLoad = async ({ params, fetch }) => {
    ...
    const song_relations = data.song ? (await client.GET('/api/tag/song_relations', { fetch, params: { query: {
    ...
    return {
        ...
        song_links: data.song ? [
            ...
            { pathname: `tag/${params.tag_slug}/song_tags`, title: m.empty_legal_chicken_taste() },
            { pathname: `tag/${params.tag_slug}/edit`, title: m.minor_crisp_cobra_list() }
        ] : null,
        tag: data,
        song_relations
