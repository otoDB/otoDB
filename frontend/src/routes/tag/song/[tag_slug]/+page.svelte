...
<Section title={m.mild_loud_shad_enchant({ type: "Song Tag", name: data.tag.name })}
    ...
    <h2>{m.mild_loud_shad_enchant({type: m.plane_awful_bobcat_spark(), name: SongTagCategory[data.tag.category]()})}</h2>
...
<Section title={m.same_broad_haddock_pinch()}>
    <CommentTree comments={data.comments} user={data.user ?? null} model="tagsong" pk={data.tag.id!}/>
</Section>
